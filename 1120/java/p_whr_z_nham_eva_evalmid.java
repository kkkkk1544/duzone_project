...
@Service("p_whr_z_nham_eva_evalmid")
public class p_whr_z_nham_eva_evalmid extends DzErpiuServiceBase {
	...
	protected DzResponseModel executeInternal(DzRequestModel requestModel) throws Exception {
		...
		this.logger.info("ORDER executeInternal start ;;; p_whr_z_nham_eva_evalmid");
		// TODO Auto-generated method stub
		...
		try{		
			...
			if(StringUtil.equals(requestModel.getActionName(), "empList")){	//목표대상
				...
				DataTable dt = this.GetEmpList(cd_company, ym_evlu, cd_evunit, cd_evgroup);
				this.displayDataTable(dt);
				responseModel.setResultDataTable(dt);
				
			}else if(StringUtil.equals(requestModel.getActionName(), "list") ){  //목표항목
				
				String no_subjectE = requestModel.getParameterValue("no_subjectE");
				String ym_evlu = requestModel.getParameterValue("ym_evlu");
				String cd_evunit = requestModel.getParameterValue("cd_evunit");
				String cd_evgroup = requestModel.getParameterValue("cd_evgroup");
				
				//this.logger.info(cd_sysdef + ";;;;;;;;;;;;;;;;");
				DataTable dt = this.GetLsit(cd_company, no_subjectE, ym_evlu, cd_evunit, cd_evgroup);
				this.displayDataTable(dt);
				responseModel.setResultDataTable(dt);
				
			}else if(StringUtil.equals(requestModel.getActionName(), "MidtList") ){  //피평가자 의견
				...
				DataTable dt = this.GetMidtLsit(cd_company, no_subjectE, ym_evlu, cd_evunit, cd_evgroup, no_itemseq, no_seq);
				this.displayDataTable(dt);
				responseModel.setResultDataTable(dt);
				
			}else if(StringUtil.equals(requestModel.getActionName(), "dataSave") ){  //평가자 의견
				...
				String no_itemseq = requestModel.getParameterValue("no_itemseq");
				String no_seq = requestModel.getParameterValue("no_seq");
				logger.info(cd_company+";;;"+no_emp+";;;"+ ym_evlu+";;;"+ cd_evunit+";;;"+ cd_evgroup+";;;"+ no_itemseq+";;;"+ no_seq+";;;"+ no_subjectE);
				
				this.SaveData(cd_company, no_emp, no_subjectE, ym_evlu, cd_evunit, cd_evgroup, no_itemseq, no_seq,requestModel.getGridData());
			}
	...
	@Transactional
	private DataTable GetEmpList(String cd_company, String ym_evlu, String cd_evunit, String cd_evgroup){
		DataTable dt = null;
		logger.info(ym_evlu + ";;;;;;;;"+cd_company+";;;;;;;;;;;"+cd_evunit+";;;;;;;;;;;;; " + cd_evgroup);
		try{
			SqlPack so = new SqlPack();
			so.setStoreProcedure(true);
			so.setSqlText("UP_WHR_Z_NHAM_EVA_RSLT_APP_GS");		
			...
			//so.getInParameters().put("P_M_CD_EVGROUP", cd_evgroup);
	...
	@Transactional
	private DataTable GetLsit(String cd_company,String no_subjectE,String ym_evlu, String cd_evunit, String cd_evgroup){
		DataTable dt = null;
		logger.info(ym_evlu + ";;;;;;;;;"+cd_evunit+";;;;;;;;;; " + cd_evgroup+";;;;;;;;;; " + no_subjectE);
		try{
			SqlPack so = new SqlPack();
			so.setStoreProcedure(true);
			...
			so.getInParameters().put("P_YM_EVLU", ym_evlu);
			so.getInParameters().put("P_CD_EVUNIT", cd_evunit);
			so.getInParameters().put("P_NO_EMP", no_subjectE);
			so.getInParameters().put("P_YN_APPROVE", "300");
			so.getInParameters().put("P_CD_EVGROUP", cd_evgroup);
	...
	//평가자의견
	@Transactional
	private DataTable GetMidtLsit(String cd_company,String no_subjectE,String ym_evlu, String cd_evunit, String cd_evgroup, String no_itemseq, String no_seq){
		DataTable dt = null;
		logger.info(ym_evlu + ";;;;;;;;;;;;"+no_itemseq+";;;;;;;;;;;"+no_subjectE+";;;;;;;;;;; " + no_seq);
		try{
			SqlPack so = new SqlPack();
			so.setStoreProcedure(true);
...
//				so.getInParameters().put("P_CD_INPUT", "200");		//입력구분  (100 : 성과기록, 200 : 중간평가)
	...
	@Transactional
	private void SaveData(String cd_company,String no_emp,String no_subjectE, String ym_evlu,String cd_evunit,String cd_evgroup,
		...
		try{
			...
			if(add != null && addCnt > 0){
				for(Map<String, Object> item : add){
					//logger.debug(";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"+Convert.toString(item.get("TP_PROGRESS")));
					SqlPack so = new SqlPack();
					so.setStoreProcedure(true);
					...
					so.getInParameters().put("P_NO_ITEMSEQ",  no_itemseq);	//항목순번
					so.getInParameters().put("P_CD_INPUT",    "100");		//입력구분 (평가자 100, 피평가자 200)
					so.getInParameters().put("P_NO_EMP",      no_subjectE);	//해당코멘튿의 피평가자 no_emp
					so.getInParameters().put("P_NO_EMPCMMT",  no_emp);		//코멘트 작성자사번
					...
					so.getInParameters().put("P_TP_PROGRESS", Convert.toString(item.get("TP_PROGRESS")));			//도달수준
...
//				logger.debug(";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;     UPDATE");
				...
				for(Map<String, Object> item : update){
					...
					pack.getInParameters().put("P_CD_INPUT",    "100");			//입력구분 (평가자 100, 피평가자 200)
					pack.getInParameters().put("P_NO_EMP",      no_subjectE);	//평가자 사번?   작성자사번
					...
					pack.getInParameters().put("P_TP_PROGRESS", Convert.toString(item.get("TP_PROGRESS")));			//도달수준
					pack.getInParameters().put("P_ID_UPDATE",   id_user);		//수정자ID
			...
			if(delete != null && deleteCnt > 0){
				for(Map<String, Object> item : delete){
					...
					sp.getInParameters().put("P_CD_INPUT", "100");			//입력구분 (평가자 100, 피평가자 200)
					sp.getInParameters().put("P_NO_EMP", no_subjectE);		//사원번호
