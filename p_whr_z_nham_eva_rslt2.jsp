...
<tiles:insertDefinition name="basicTemplate">
	<tiles:putAttribute name="content">
		<!--   
			...
			수정자 : 
			수정일 :
			수정내용 : p_whr_z_nham_eva_rslt2.jsp
		 -->
...
//			getCDevtype();
		...
		function gridList1() {  
			grid = dews.ui.grid("#grid_list", {
	            ...
	            save: function(e) {
	            	// 달성도가 수정되었는지 확인
			        if (e.values.hasOwnProperty('NUM_RSLT_1')) { // 1차달성도가 수정되었는지 체크
	            		if(e.model.CD_RSLT_1 != e.values.NUM_RSLT_1) {
		                	e.model.CD_RSLT_1 = e.values.NUM_RSLT_1; // 다른 입력 가능 셀의 값을 변경시킵니다.
		            		e.model.NUM_RSLT_1 = e.values.NUM_RSLT_1;
		                    e.model.NUM_CVTRSLT_1 = e.values.NUM_RSLT_1; // 다른 입력 가능 셀의 값을 변경시킵니다.
		            		isSave = {result : true, currentNo : e.model.NO_EMP}
	            		}else{
		            		isSave = {result : false, currentNo : e.model.NO_EMP}
	            		}
	            		...
	            		if(e.model.CD_RSLT_2 != e.values.NUM_RSLT_2) {
		                	e.model.CD_RSLT_2 = e.values.NUM_RSLT_2; // 다른 입력 가능 셀의 값을 변경시킵니다.
		            		e.model.NUM_RSLT_2 = e.values.NUM_RSLT_2;
		                    e.model.NUM_CVTRSLT_2 = e.values.NUM_RSLT_2; // 다른 입력 가능 셀의 값을 변경시킵니다.
		            		isSave = {result : true, currentNo : e.model.NO_EMP}
	            		}else{
		            		isSave = {result : false, currentNo : e.model.NO_EMP}
	            		}
	            ...
	            save: function(e) {
	            	...
	            	if (e.values.hasOwnProperty('NM_SCALED_1')) { // 1차달성도가 수정되었는지 체크
	            		if(e.model.CD_RSLT_1 != e.values.NM_SCALED_1){
	            			if(e.model.NM_SCALED2 == ""){
			            		if(e.values.NM_SCALED_1 > e.model.NUM_ITEM) {
			        				dews.alert("달성도는 배점을 초과 할 수 없습니다.");
				            		isSave = {result : false, currentNo : e.model.NO_EMP}
			            		}
			                	e.model.CD_RSLT_1 = cdScaleh; // 다른 입력 가능 셀의 값을 변경시킵니다.
			            		e.model.NUM_RSLT_1 = e.values.NM_SCALED_1;
			                    e.model.NUM_CVTRSLT_1 = e.values.NM_SCALED_1; // 다른 입력 가능 셀의 값을 변경시킵니다.
			            		isSave = {result : true, currentNo : e.model.NO_EMP}
	            			}
	            			else{	            				
			            		var result1 = rsltScmList(cdScaleh, e.values.NM_SCALED_1, e.model.NUM_ITEM);
            			...
            			if(e.model.NM_SCALED1 == ""){
		                	...
		                	e.model.CD_RSLT_2 = cdScaleh; // 다른 입력 가능 셀의 값을 변경시킵니다.
		            		e.model.NUM_RSLT_2 = e.values.NM_SCALED_2;
		                    e.model.NUM_CVTRSLT_2 = e.values.NM_SCALED_2; // 다른 입력 가능 셀의 값을 변경시킵니다.
		            		isSave = {result : true, currentNo : e.model.NO_EMP}
            			}
            			else{	            				
		            		var result2 = rsltScmList(cdScaleh, e.values.NM_SCALED_2, e.model.NUM_ITEM);
