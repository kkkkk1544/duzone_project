...
<tiles:putAttribute name="content">
	<!--   
		...
		수정자 : 
		수정일 :
		수정내용 : p_whr_z_nham_goal_appr_adm.jsp
	 -->
    <!-- 버튼영역 시작 -->
...
<!-- 								data-dews-data-url="~/Data/help_p_whr_z_nham_eva_yyyymm/yyyymm?CD_EVTYPE=300" -->
   ...
   <div class="dews-grid-panel" style="width:100%;height:100%;">
	...
	dews.ready(function(){
		var datasource, datasource2,  datasource4, admin;
        ...
        grid = dews.ui.grid("#grid_list", {
	     ...
	     grid_commt = dews.ui.grid("#grid_commt", {
	          ...
	          change: function(e){
			 		...
			 		if(select_commt_rowData.NO_EMPCMMT == no_emp_c && "100" == $("#CD_STATE").val()){
	     ...
	     $('#btn_save').on('click', function (e){
	    	 if (datasource2.getDirtyDataCount() > 0) {
	    		 dews.confirm(dews.localize.get("저장하시겠습니까?")).yes(function(){
	    			 ...
	    			 if(datasource2.getDirtyDataCount() > 0){
	    				 datasource2.batchSave(dews.urlHelper.getDataUrl("p_whr_z_nham_goal_appr_adm", "AdminSave_cmmt"), {
    					 async: false,
	 						data: {
	 							NoEmp      : dews.ui.codepicker('#NoEmp').code(),  //관리자 추가
	        ...
	        var cd_evgroup = app_rowData.CD_EVGROUP;
	 		...
	 		if(app_rowData){
	 			dews.confirm(dews.localize.get("반려 하시겠습니까?")).yes(function(){
		 			dews.ajax.data( dews.url.getDataUrl("p_whr_z_nham_goal_appr_adm", "AdminApp"), {
		 		        async: false,
		 		        data:{
		 		        	ym_evlu	   : ym_evlu,
		 		        	cd_evunit  : cd_evunit,
	   ...
	   function ymSelect(admin){
	    ...
	    //pageinit 시 디폴트데이더 셋팅 commonDataProvider.class-jar
	 	...
	 	function SelectCmmt(){
	        ...
	        dews.ajax.data( dews.url.getDataUrl("p_whr_z_nham_goal_appr_adm", "AdminList_cmmt"), {
 		        async: false,
 		        data:{
 		        	ym_evlu	   : ym_evlu,
 		        	cd_evunit  : cd_evunit,
	 	...
	 	function SelectData(){
			...
			var NoEmp = dews.ui.codepicker('#NoEmp').code();
 		    ...
 		    dews.ajax.data( dews.url.getDataUrl("p_whr_z_nham_goal_appr_adm", "AdminList"), {
 		        async: false,
 		        data:{
 		        	ym_evlu	   : ym_evlu,
 		        	cd_evunit  : cd_evunit,
 		    	...
 		    	if(result.length){
					...
					//관리자추가 변경 로그인한 대상자의 사번으로 변경해줌		
					if(result[0].NO_TEAM != NoEmp){
	 		    		$('#btn_save').prop('disabled','disabled');
	 	...
	 	function SelectSubject(){
			...
			dews.ajax.data( dews.url.getDataUrl("p_whr_z_nham_goal_appr_adm", "AdminList_subject"), {
 		        async: false,
 		        data:{
 		        	ym_evlu	   : ym_evlu,
 		        	cd_evunit  : cd_evunit,
